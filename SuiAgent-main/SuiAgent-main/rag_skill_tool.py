    ...
    async def retrieve(self, query: str) -> Dict[str, Union[str, List[Dict[str, str]], List[str]]]:
        ...
        for file_path in target_files:
            ...
            for attempt in range(self.max_attempts):
                ...
                if sufficient:
                    print(f"{query} 上下文已充分")
                    break

                if attempt == self.max_attempts - 1:
                    print(f"{query}已达到最大尝试次数")
                    break

                if not new_keywords:
                    print(f"{query}无法生成有效新关键词")
                    break
            ...
            if not file_fragments:
                print(f"{query}使用LLM阅读: {file_path.name}")
