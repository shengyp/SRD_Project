...
def save_correct_predictions(correct_indices, text_csv_path, emoji_csv_path, output_path):
    """保存预测正确的样本，将post_sequence拆分为贴文序列和微表情序列"""
    ...
    for idx in correct_indices:
        ...
        correct_samples.append({
            'user_id': text_row['user_id'],
            'created_utc': text_row['created_utc'],
            '贴文序列': text_row['post_sequence'],
            '微表情序列': emoji_row['post_sequence'],
            'suicide_risk': text_row['suicide_risk']
        })
...
def eval_epoch_bert_emoji(model, dataloader, device, args):
    ...
    acc = accuracy_score(label_result, out_result)
    f1_macro = f1_score(label_result, out_result, average='macro')
    f1_weighted = f1_score(label_result, out_result, average='weighted')
    GP, GR, FS, _ = gr_metrics(out_result, label_result)
    return total_loss / len(dataloader), acc, GP, GR, FS, f1_weighted
...
def test_model_bert_emoji(model, dataloader, device, test_indices=None):
    ...
    return acc, GP, GR, FS, OE, out_result, label_result, correct_indices
...
class EarlyStopping:
    ...
    def __call__(self, val_f1, model):
        """
        Args:
            val_f1: 加权F1分数 (weighted F1 score)
            model: 模型实例
        """
        score = val_f1
    ...
    def save_checkpoint(self, val_f1, model):
        """保存模型检查点"""
        if self.verbose:
            print(f"Weighted F1 提升 ({self.val_f1_max:.6f} --> {val_f1:.6f})，保存模型...")
...
def parse_args():
    ...
    parser.add_argument("--bert_pkl", type=str, default='data/bigdata_bert.pkl')
    parser.add_argument("--emoji_csv", type=str, default='data/bigdata_emoji_batch.csv')
    ...
    parser.add_argument("--max_posts", type=int, default=300, help='最大帖子序列长度')
...
def train_bert_emoji():
    ...
    for epoch in range(args.epochs):
        ...
        val_loss, val_acc, val_GP, val_GR, val_FS, val_f1_weighted = eval_epoch_bert_emoji(
            model, val_loader, device, args)
        print(f"Val Loss: {val_loss:.4f} | Acc: {val_acc:.4f} | GP: {val_GP:.4f} | GR: {val_GR:.4f} | FS: {val_FS:.4f} | Weighted F1: {val_f1_weighted:.4f}")
        
        scheduler.step()
        # 使用Weighted F1作为早停指标
        early_stopping(val_f1_weighted, model)
        if early_stopping.early_stop:
            print("早停触发")
            break
    
    print(f"\n训练完成，最佳Weighted F1: {early_stopping.val_f1_max:.4f}")
    ...
    if args.mode in ['test', 'train_test']:
        ...
        test_acc, test_GP, test_GR, test_FS, test_OE, preds, labels_test, correct_indices = test_model_bert_emoji(
        ...
        print(f"Weighted F1: {f1_score(labels_test, preds, average='weighted'):.4f}")
        print("=" * 50)
        
        print_class_statistics(preds, labels_test, class_num=args.class_num)
        
        text_csv_path = 'data/bigdata.csv'
        emoji_csv_path = 'data/bigdata_emoji_batch.csv'
        output_path = 'data/correct_predictions.csv'
