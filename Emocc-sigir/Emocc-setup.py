...
def save_correct_predictions(correct_indices, text_csv_path, emoji_csv_path, output_path):
    ...
    for idx in correct_indices:
        ...
        correct_samples.append({
            'Post_Text': text_row['Post'],
            'Post_Emoji': emoji_row['Post'],
            'Label': text_row['Label']
        })
...
def train_epoch_bert_emoji(model, dataloader, optimizer, device, args):
    ...
    acc_bin, precision, recall, f1_bin = binary_metrics(out_result, label_result)
    return total_loss / len(dataloader), acc, acc_bin, precision, recall, f1_bin
...
def test_model_bert_emoji(model, dataloader, device, test_indices=None):
    ...
    return acc_bin, precision, recall, f1_bin, out_result, label_result, correct_indices
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_f1, model):
        if self.verbose:
            print(f"F1 提升 ({self.val_f1_max:.6f} --> {val_f1:.6f})，保存模型...")
...
def parse_args():
    ...
    parser.add_argument("--bert_pkl", type=str, default='data/sigir_bert.pkl')
    parser.add_argument("--emoji_csv", type=str, default='data/sigir_emojis.csv')
    ...
    parser.add_argument("--max_posts", type=int, default=1, help='固定为1（每个样本只有一个帖子）')
...
def train_bert_emoji():
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc, train_acc_bin, train_precision, train_recall, train_f1 = train_epoch_bert_emoji(
            model, train_loader, optimizer, device, args)
        print(f"Train Loss: {train_loss:.4f} | Acc: {train_acc:.4f} | Binary_Acc: {train_acc_bin:.4f} | Precision: {train_precision:.4f} | Recall: {train_recall:.4f} | F1: {train_f1:.4f}")
        
        val_loss, val_acc, val_acc_bin, val_precision, val_recall, val_f1 = eval_epoch_bert_emoji(
            model, val_loader, device, args)
        print(f"Val Loss: {val_loss:.4f} | Acc: {val_acc:.4f} | Binary_Acc: {val_acc_bin:.4f} | Precision: {val_precision:.4f} | Recall: {val_recall:.4f} | F1: {val_f1:.4f}")
    ...
    print(f"\n训练完成，最佳F1: {early_stopping.val_f1_max:.4f}")
    ...
    if args.mode in ['test', 'train_test']:
        ...
        test_acc_bin, test_precision, test_recall, test_f1, preds, labels_test, correct_indices = test_model_bert_emoji(
            model, test_loader, device, test_indices=test_indices)
        
        print("\n" + "=" * 50)
        print("最终测试结果:")
        print("=" * 50)
        print(f"Binary_Accuracy: {test_acc_bin:.4f}")
        ...
        emoji_csv_path = 'data/sigir_emojis.csv'
        output_path = 'data/sigir_correct_predictions.csv'
